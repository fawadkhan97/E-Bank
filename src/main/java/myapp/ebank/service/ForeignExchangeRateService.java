...
public class ForeignExchangeRateService {
    ...
    public ResponseEntity<Object> getForeignExchangeRateBetweenDates(@RequestParam java.util.Date startDate, @RequestParam java.util.Date endDate, HttpServletRequest httpServletRequest) throws ParseException {
        try {
            ...
            if (!foreignExchangeRates.isEmpty()) {
                return new ResponseEntity<>(ResponseMapping.ApiReponse(HttpStatus.OK, "found today foreign exchange rates ", httpServletRequest.getRequestURI(), foreignExchangeRates), HttpStatus.OK);

            } else
                return new ResponseEntity<>(ResponseMapping.ApiReponse(HttpStatus.NOT_FOUND, "Could not get foreign rate ...", httpServletRequest.getRequestURI(), null), HttpStatus.NOT_FOUND);
        } catch (Exception e) {
            log.info(
